...
import { getAboutPage, getDocuments, getPartners } from '../services/strapi';
...
export default function About() {
  ...
  const [aboutData, setAboutData] = React.useState<null | {
    ...
    mission_h2?: string;
    mission_stats?: { value: string; label: string; order: number }[];
    image_overlay_org?: string;
    image_overlay_address?: string;
    ...
    history_section_subtitle?: string;
    leadership_section_title?: string;
    leadership_section_subtitle?: string;
    documents_preview_title?: string;
    documents_preview_subtitle?: string;
    partners_preview_title?: string;
    partners_preview_subtitle?: string;
  }>(null);

  const [docsPreview, setDocsPreview] = React.useState<{ id: number; title: string; file_type?: string; description?: string }[]>([]);
  const [partnerChips, setPartnerChips] = React.useState<{ id: number; name: string }[]>([]);

  React.useEffect(() => {
    getAboutPage(locale).then(setAboutData).catch(() => undefined);
    getDocuments(locale).then((items: any[]) => {
      if (Array.isArray(items)) setDocsPreview(items.slice(0, 6));
    }).catch(() => undefined);
    getPartners(locale).then((items: any[]) => {
      if (Array.isArray(items)) setPartnerChips(items.slice(0, 6));
    }).catch(() => undefined);
  }, [locale]);
  ...
  const missionStats = aboutData?.mission_stats?.length
    ? [...aboutData.mission_stats].sort((a, b) => a.order - b.order)
    : [];
    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="bg-gray-50 border-b border-gray-200 py-10">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-1">{aboutData?.page_title || ''}</h1>
          <p className="text-gray-600">{aboutData?.page_subtitle || ''}</p>
          {/* Sub-nav tabs */}
      ...
      <div className="container mx-auto px-4 md:px-6 py-14 space-y-20">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              {aboutData?.mission_heading && (
                <p className="text-sm font-bold text-dnu-blue uppercase tracking-wider mb-3">{aboutData.mission_heading}</p>
              )}
              {aboutData?.mission_h2 && (
                <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 mb-5 leading-tight">{aboutData.mission_h2}</h2>
              )}
              {aboutData?.structure_description && (
                <p className="text-gray-600 leading-relaxed mb-4">{aboutData.structure_description}</p>
              )}
              {aboutData?.history && (
                <p className="text-gray-600 leading-relaxed mb-8">{aboutData.history}</p>
              )}
              {(aboutData?.mission_checklist?.length ?? 0) > 0 && (
                <ul className="space-y-3 mb-8">
                  {aboutData!.mission_checklist!.map((c) => (
              ...
              <div className="aspect-4/3 bg-linear-to-br from-dnu-blue to-dnu-dark rounded-3xl overflow-hidden shadow-2xl">
                ...
                <div className="absolute inset-0 flex items-end p-8">
                  <div className="bg-white/90 backdrop-blur-sm rounded-xl p-4 w-full">
                    <p className="text-dnu-dark font-bold text-sm">{aboutData?.image_overlay_org || ''}</p>
                    <p className="text-gray-600 text-xs mt-1">{aboutData?.image_overlay_address || ''}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {activeTimeline.length > 0 && (
          <section>
            <h2 className="text-2xl font-bold text-gray-900 mb-3">{aboutData?.history_section_title || ''}</h2>
            <p className="text-gray-600 mb-10">{aboutData?.history_section_subtitle || ''}</p>
            <div className="relative">
        ...
        {activeLeadership.length > 0 && (
          <section>
            <div className="flex items-end justify-between mb-8">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-1">{aboutData?.leadership_section_title || ''}</h2>
                <p className="text-gray-600 text-sm">{aboutData?.leadership_section_subtitle || ''}</p>
              </div>
        ...
        <section>
          <div className="mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-1">{aboutData?.documents_preview_title || ''}</h2>
            <p className="text-gray-600 text-sm">{aboutData?.documents_preview_subtitle || ''}</p>
          </div>
          {docsPreview.length > 0 && (
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
              {docsPreview.map((doc) => (
                <Link key={doc.id} to="/documents" className="bg-gray-50 border border-gray-200 rounded-xl p-5 flex items-start gap-4 hover:border-dnu-blue/40 hover:shadow-sm transition-all group">
        ...
        {partnerChips.length > 0 && (
          <section>
            <div className="flex items-end justify-between mb-8">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-1">{aboutData?.partners_preview_title || ''}</h2>
                <p className="text-gray-600 text-sm">{aboutData?.partners_preview_subtitle || ''}</p>
              </div>
            ...
            <div className="flex flex-wrap gap-4">
              {partnerChips.map((p) => (
                <div key={p.id} className="flex items-center justify-center h-16 px-5 border border-gray-200 rounded-xl bg-gray-50 text-xs text-gray-500 font-medium text-center hover:border-dnu-blue/40 hover:bg-white transition-colors min-w-35">
